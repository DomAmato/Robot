...
import com.dom.rjm.actions.SetBlocksNBT;
import com.dom.rjm.actions.SetBlocksState;
import com.dom.rjm.events.MCEventHandler;
import com.dom.rjm.network.SocketEvent;
import com.dom.rjm.util.Location;
import com.dom.rjm.util.SetDimension;
import com.dom.rjm.util.Vec3w;
...
public class Python2MinecraftApi {
		...
		static class HitDescription {
			...
			public HitDescription(World[] worlds, PlayerInteractEvent event) {
				Vec3i pos = Location.encodeVec3i(worlds, event.getEntityPlayer().getEntityWorld(),
		...
		protected static void entityGetTile(int id) {
			...
			if (e != null) {
				...
				Python2MinecraftApi.sendLine("" + Python2MinecraftApi.trunc(pos.x) + ","
						+ Python2MinecraftApi.trunc(pos.y) + "," + Python2MinecraftApi.trunc(pos.z));
			}
		}
					...
					(String args, Scanner scan, MCEventHandler eventHandler) -> {
						if (Python2MinecraftApi.includeNBTWithData) {
							Python2MinecraftApi.sendLine(
									eventHandler.describeBlockState(Python2MinecraftApi.getBlockLocation(scan)));
						} else {
						...
						for (int y = y1; y <= y2; y++) {
							for (int x = x1; x <= x2; x++) {
								for (int z = z1; z <= z2; z++) {
									...
									if (Python2MinecraftApi.includeNBTWithData) {
										out.append(eventHandler.describeBlockState(pos).replace("&", "&amp;")
												.replace("|", "&#124;"));
									} else {
										out.append("" + eventHandler.getBlockId(pos) + ","
												+ eventHandler.getBlockMeta(pos));
									}
								}
							}
						}
		...
		protected static void spawnParticle(Scanner scan) {
			if (!Python2MinecraftApi.useClientMethods) {
				...
				if (particle == null) {
					...
					((WorldServer) pos.getWorld()).spawnParticle(particle, false, pos.x, pos.y, pos.z, count, dx, dy,
							dz, speed, extras);
				}
			} else {
				String particleName = scan.next();
