...
public class MagnetScreen extends Show {
	...
	public void setup() {
		...
		buttonEntries.add(new MultiComponentListEntry()
				...
				.registerComponent(new TextLabel(0, 0, (int) (panel.getWidth()-10), 10, Color.black, "Activated Robots Nearby", TextAlignment.CENTER), 5, 10)
				...
				buttonEntries.add(new MultiComponentListEntry()
						...
						.registerComponent(new TextLabel(0, 0, (int) (panel.getWidth() * .8), 10,
								"Pos - X:" + (int) (robot.posX) + " Y:" + (int) (robot.posY) + " Z:"
										+ (int) (robot.posZ)),
								5, 17)
						.registerComponent(new Button(0, 0, (int) (panel.getWidth() * .3), 28, "Warp")
								.setClickListener(btn -> {
									NetworkManager.sendToServer(new MessageTeleportRobot(robot.getEntityId(), pos, state.getValue(BlockHorizontal.FACING)));
