...
public class RobotAPIHandler extends APIHandler {
	...
	// camera.setNormal(id) and camera.setFollow(id) uses spectating, and so it
	// moves the
	...
	public RobotAPIHandler(MCEventHandler eventHandler, PrintWriter writer) throws IOException {
	...
	protected void entityGetPos(int id) {
		...
		if (e != null) {
			...
			while (w != e.getEntityWorld()) {
				...
				// getting w and pos0.
				// To be somewhat safe, let's sleep for approximately a server
				// tick and get
				// everything again.
				try {
					Thread.sleep(50);
			...
			for (int y = y1; y <= y2; y++) {
				for (int x = x1; x <= x2; x++) {
					for (int z = z1; z <= z2; z++) {
						...
						if (includeNBTWithData) {
							out.append(
									eventHandler.describeBlockState(pos).replace("&", "&amp;").replace("|", "&#124;"));
						} else {
